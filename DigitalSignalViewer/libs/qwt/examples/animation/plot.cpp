...
{
public:
    void setTransformation(const QTransform &transform) { d_transform = transform; }

    virtual void updateSamples(double phase) { setSamples(d_transform.map(points(phase))); }
...
public:
    Curve2()
    {
        ...
        setSymbol(
            new QwtSymbol(QwtSymbol::Ellipse, QColor(Qt::gray), QColor(Qt::yellow), QSize(5, 5)));
    }
